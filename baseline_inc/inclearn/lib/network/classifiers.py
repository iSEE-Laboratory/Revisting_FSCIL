...
class CosineClassifier(nn.Module):
    ...
    def forward(self, features):
        ...
        if self.distance == "cosine":
            raw_similarities = distance_lib.cosine_similarity(features, weights) * 12
...
class ETFCls(CosineClassifier):
    def __init__(self, num_featrues, *args, **kwargs):
        super(ETFCls, self).__init__(num_featrues, 'cuda', *args, **kwargs)
        self.neck = MLPFFNNeck(num_featrues, 256)
        self.head = ETFHead(100, 256)
        self.classifier_type = 'ETF'
        self.num_classes = 60
        print(self.neck)

    def forward(self, features):
        feat_2 = self.neck(features)
        logits = self.head(feat_2)[:, :self.num_classes]
        return {'logits': logits}

    # def add_custom_weights(self, *args, **kwargs):
    #     return

    # def add_classes(self, n_classes):
    #     self.num_classes += n_classes
    #     print(f'current_num_cls:{self.num_classes}')
