...
def _accuracy_per_task(ypreds, ytrue, init_task_size=0, task_size=10, topk=1):
    ...
    all_acc["total"] = _accuracy(ypreds, ytrue, topk=topk)
    ...
    if task_size is not None:
        ...
        if init_task_size > 0:
            ...
            all_acc[label] = _accuracy(ypreds[idxes], ytrue[idxes], topk=topk)
...
def _old_accuracy(ypreds, ytrue, task_size):
    """Computes accuracy for the whole test & per task.
    ...
    return _accuracy(ypreds[old_class_indexes], ytrue[old_class_indexes], topk=1)


def _new_accuracy(ypreds, ytrue, task_size):
    """Computes accuracy for the whole test & per task.
    ...
    return _accuracy(ypreds[new_class_indexes], ytrue[new_class_indexes], topk=1)


def _accuracy(output, targets, topk=1):
...
def _incremental_accuracy(accuracies):
...
def _forward_transfer(accuracies):
...
class MetricLogger:
    ...
    def log_task(self, ypreds, ytrue, task_size, zeroshot=False):
        self.metrics["accuracy"].append(
            _accuracy_per_task(ypreds, ytrue, task_size=10, topk=1)
        )  # FIXME various task size
        self.metrics["accuracy_top5"].append(
            _accuracy_per_task(ypreds, ytrue, task_size=None, topk=5)
        )
        self.metrics["accuracy_per_class"].append(
            _accuracy_per_task(ypreds, ytrue, task_size=1, topk=1)
        )
        self.metrics["incremental_accuracy"].append(_incremental_accuracy(self.metrics["accuracy"]))
        self.metrics["incremental_accuracy_top5"].append(
            _incremental_accuracy(self.metrics["accuracy_top5"])
        )
        self.metrics["forgetting"].append(_forgetting(self.metrics["accuracy"]))
        ...
        if zeroshot:
            ...
            self.metrics["seen_classes_accuracy"].append(
                _accuracy(ypreds[seen_classes_indexes], ytrue[seen_classes_indexes])
            )
            unseen_classes_indexes = np.where(
                ytrue >= sum(self.increments[:self._task_counter + 1])
            )[0]
            self.metrics["unseen_classes_accuracy"].append(
                _accuracy(ypreds[unseen_classes_indexes], ytrue[unseen_classes_indexes])
            )

        if self._task_counter > 0:
            self.metrics["old_accuracy"].append(_old_accuracy(ypreds, ytrue, task_size))
            self.metrics["new_accuracy"].append(_new_accuracy(ypreds, ytrue, task_size))
